    ...
    <table cellpadding="0" cellspacing="1" border="0" width="95%" class="formtable">
        ...
        <tr>
            <th style="width:100px;">教员：</th>
             <td><input type="text" id="UserName" name="UserName" runat="server" /><input type="button" class="mybutton" value="选择" onclick="Sel()"/></td>        
<%--            <td><input type="text" class="mymember" runat="server" unit="0" dept="0" station="0" user="1" group="0" id="ChargeLeader" name="ChargeLeader" /></td>--%>
        ...
        <tr>
            <th style="width:80px;">学员骨干：</th>
              <td><input type="text" id="Lead" name="Lead" runat="server" /><input type="button" class="mybutton" value="选择" onclick="SelLeader()"/></td>        
    ...
    <div style="width:95%; margin:10px auto 10px auto; text-align:center;">
        <input type="hidden" runat="server" unit="0" dept="0" station="0" user="1" group="0" id="Leader" name="Leader" />
        <input type="hidden" runat="server" unit="0" dept="0" station="0" user="1" group="0" id="ChargeLeader" name="ChargeLeader" />
        <input type="button" class="mybutton" onclick="window.location='BodyAdd.aspx'+'<%=Request.Url.Query%>';" value="添加单位/期班" id="addChild" />
        ...
        <input type="hidden" id="QbId" value="<%=Request.QueryString["id"] %>" />
    ...
    <script type="text/javascript">
        window.onload = function(){
            ...
            var valueL = $("#Leader").val();
            if (valueL && valueL.length > 0) {
                $.ajax({
                    url: (top.rootdir || "") +"/Controls/SelectMember/GetNames.ashx?values=" + valueL, type: "get", async: false, cache: false, success: function (txt) {
                        $("#Lead").val(txt);
                    }
                });
            }
        ...
        function Sel(){
            var myUsers = $("#ChargeLeader").val();
            var qbId =  $("#QbId").val();
            var appid =  $("#appid").val();
            window.location.href="/Controls/SelectMember/SelectJiaoYuan.aspx?values=" + myUsers + '&roleid=' + qbId + '&appid=' + appid;;
        }
        ...
        function SelLeader(){
            ...
            var qbId =  $("#QbId").val();
            var appid =  $("#appid").val();
            window.location.href="/Controls/SelectMember/SelectGuGan.aspx?values=" + myLeader + '&roleid=' + qbId + '&appid=' + appid;;
        }
