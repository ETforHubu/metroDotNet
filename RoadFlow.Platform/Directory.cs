...
namespace RoadFlow.Platform
{
    ...
    {
        private RoadFlow.Data.Interface.IDirectory dataDirectory;
        private static string cacheKey = RoadFlow.Utility.Keys.CacheKeys.Directory.ToString();
        public Directory()
        {
            this.dataDirectory = Data.Factory.Factory.GetDirectory();
        }
        /// <summary>
        /// 新增
        /// </summary>
        ...
        {
            return dataDirectory.Add(model);
        }
        /// <summary>
        /// 更新
        /// </summary>
        ...
        {
            return dataDirectory.Update(model);
        }
        /// <summary>
        /// 查询所有记录
        /// </summary>
        public List<RoadFlow.Data.Model.Directory> GetAll(bool fromCache = false)
        {
            if (!fromCache)
            {
                return dataDirectory.GetAll();
            }
            else
            {
                object obj = RoadFlow.Cache.IO.Opation.Get(cacheKey);
                if (obj != null && obj is List<RoadFlow.Data.Model.Directory>)
                {
                    return obj as List<RoadFlow.Data.Model.Directory>;
                }
                else
                {
                    var list = dataDirectory.GetAll();
        ...
        {
            return dataDirectory.GetBySnumber();
        }
        /// <summary>
        /// 查询单条记录(深圳地铁1号线)
        /// </summary>
        ...
        {
            return dataDirectory.GetByTitle();
        }
        /// <summary>
        /// 查询单条记录
        /// </summary>
        public RoadFlow.Data.Model.Directory Get(Guid id, bool fromCache = false)
        {
            return fromCache ? GetAll(true).Find(p => p.ID == id) : dataDirectory.Get(id);
        }
        ...
        {
            return dataDirectory.GetParentIDAndTitle(id, title);
        }
        /// <summary>
        /// 删除
        /// </summary>
        ...
        {
            return dataDirectory.Delete(id);
        }
        /// <summary>
        /// 查询记录条数
        /// </summary>
        ...
        {
            return dataDirectory.GetRoot();
        }

        /// <summary>
        /// 查询下级记录
        /// </summary>
        public List<RoadFlow.Data.Model.Directory> GetChilds(Guid id, bool fromCache = false)
        {
            return fromCache ? getChildsByIDFromCache(id) : dataDirectory.GetChilds(id);
        }

        /// <summary>
        /// 查询下级记录
        /// </summary>
        public List<RoadFlow.Data.Model.Directory> GetChilds(string code, bool fromCache = false)
        {
            return code.IsNullOrEmpty() ? new List<RoadFlow.Data.Model.Directory>() :
                fromCache ? getChildsByCodeFromCache(code) :
                dataDirectory.GetChilds(code.Trim());
        }

        private List<RoadFlow.Data.Model.Directory> getChildsByCodeFromCache(string code)
        {
            var list = GetAll(true);
            var dict = list.Find(p => string.Compare(p.Code, code, true) == 0);
            return dict == null ? new List<RoadFlow.Data.Model.Directory>() : list.FindAll(p => p.ParentID == dict.ID).OrderBy(p => p.Sort).ToList();
        }

        private List<RoadFlow.Data.Model.Directory> getChildsByIDFromCache(Guid id)
        {
            var list = GetAll(true);
        ...
        /// <returns></returns>
        public List<RoadFlow.Data.Model.Directory> GetAllChilds(string code, bool fromCache)
        {
            if (code.IsNullOrEmpty()) return new List<RoadFlow.Data.Model.Directory>();
            var dict = GetByCode(code, fromCache);
            if (dict == null) return new List<RoadFlow.Data.Model.Directory>();
        ...
        {
            return dataDirectory.GetFirstChilds(id);
        }
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public List<RoadFlow.Data.Model.Directory> GetAllChilds(Guid id, bool fromCache = false)
        {
        ...
        private void addChilds(List<RoadFlow.Data.Model.Directory> list, Guid id, bool fromCache = false)
        {
        ...
        {
            return dataDirectory.GetParent(id);
        }
        ...
        {
            return dataDirectory.HasChilds(id);
        }
        ...
        {
            return dataDirectory.GetMaxSort(id);
        }
        ...
        {
            return dataDirectory.UpdateSort(id, sort);
        }
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public RoadFlow.Data.Model.Directory GetByCode(string code, bool fromCache = false)
        {
            return code.IsNullOrEmpty() ? null :
                fromCache ? GetAll(true).Find(p => string.Compare(p.Code, code, true) == 0) : dataDirectory.GetByCode(code.Trim());
        }
        ...
        /// <returns></returns>
        private int getParentCount(List<RoadFlow.Data.Model.Directory> dictList, RoadFlow.Data.Model.Directory dict)
        {
            int parent = 0;
            RoadFlow.Data.Model.Directory parentDict = dictList.Find(p => p.ID == dict.ParentID);
        ...
        private string getRadios(List<RoadFlow.Data.Model.Directory> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getCheckboxs(List<RoadFlow.Data.Model.Directory> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getOptionsValue(OptionValueField valueField, RoadFlow.Data.Model.Directory dict)
        {
        ...
        /// <returns></returns>
        public string GetComboxTableHtmlByID(string id, RoadFlow.Platform.Directory.OptionValueField valueField, string defaultValue)
        {
            if (!id.IsGuid())
            {
                return "";
